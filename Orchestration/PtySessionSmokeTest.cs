...
{
    private const string Marker = "ACCEL_SESSION_OK";
    ...
    private static bool RunInteractiveCheck(TextWriter output)
    {
        ...
        try
        {
            ...
            session.WriteText("echo ENV=%ACCEL_SMOKE_TEST%\r");
            var sawEnv = collector.WaitForOccurrences("ENV=1", 1, TimeSpan.FromSeconds(10));
            output.WriteLine($"  [{(sawEnv ? "PASS" : "FAIL")}] the child sees the injected environment override (ACCEL_SMOKE_TEST=1)");
            ok &= sawEnv;
    ...
    private static bool RunRawByteInputCheck(TextWriter output)
    {
        ...
        const string RawMarker = "ACCEL_RAWBYTE_OK";
    ...
    private static bool RunJobAssignmentCheck(TextWriter output)
    {
        ...
        {
            var inShared = IsInJob(AccelJobObject.Shared, shared.ProcessId);
            output.WriteLine($"  [{(inShared ? "PASS" : "FAIL")}] pid={shared.ProcessId} IsProcessInJob(AccelJobObject.Shared) == {inShared} (default job is the statically rooted singleton)");
            ok &= inShared;
            output.WriteLine($"  [{(ReferenceEquals(shared.JobObject, AccelJobObject.Shared) ? "PASS" : "FAIL")}] the session holds a strong reference to that job for its whole lifetime (rooting guarantee (b))");
            ok &= ReferenceEquals(shared.JobObject, AccelJobObject.Shared);
        }
        ...
        var dedicated = AccelJobObject.Create();
    ...
    /// </summary>
    private static bool IsInJob(AccelJobObject job, int processId)
    {
