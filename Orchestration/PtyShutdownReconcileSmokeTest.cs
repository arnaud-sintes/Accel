...
/// <c>cmd.exe</c>, never <c>claude.exe</c>, and writes only to a temp-path <c>accel-sessions.json</c> - never the
/// user's real profile.
///
...
/// <item><b>a child that died while Accel was down is stale</b> - kill one of those children directly, bypassing
...
/// <item><b>exit path 3/3 - <c>AppDomain.ProcessExit</c></b>: a real child Accel process
...
/// <see cref="AccelJobObject"/>'s kill-on-close, which only fires later when the process's handles close. A
...
public static class PtyShutdownReconcileSmokeTest
{
    ...
    public static int Run(TextWriter output)
    {
        ...
        var failures = 0;
        var workDir = Path.Combine(Path.GetTempPath(), $"accel-p3t4-{Guid.NewGuid():N}");
    ...
    {
        output.WriteLine("== check 1/5: crash-simulated orphans classify as adoptable; a child that died while Accel was down classifies as stale ==");

        var pidRegistryPath = Path.Combine(workDir, "accel-sessions.json");
        ...
        // its sessions are never closed - the children stay alive, orphaned from Accel's bookkeeping. It is
        ...
        try
        {
            ...
            output.WriteLine("  simulating an ungraceful Accel exit: NOT calling PtyRegistry.Dispose() - the children stay alive and orphaned");
            ...
            output.WriteLine($"  killed pid {doomedPid} directly (bypassing the registry: 'the child died while Accel was down'); alive now={!reallyDead}");
    ...
    private static bool RunPidReuseGuardCheck(TextWriter output, string workDir)
    {
        ...
        var pidRegistry = new PtyPidRegistry(Path.Combine(workDir, "accel-sessions-reuse.json"));
    ...
    {
        output.WriteLine();
        output.WriteLine("== check 5/5: exit path 3/3 - a real AppDomain.ProcessExit in a real child Accel process ==");
    ...
    public static int RunProcessExitChild(TextWriter output, string verdictPath)
    {
        ...
        // AccelJobObject. It must outlive us.
    ...
    /// <summary>Re-invokes this same build: directly for an apphost (<c>Accel.exe</c>), via the muxer when the
    /// process was launched as <c>dotnet Accel.dll</c>.</summary>
