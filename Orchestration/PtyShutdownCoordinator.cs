...
public enum PtyShutdownOutcome
{
    ...
    /// <see cref="AccelJobObject"/>'s kill-on-close, which the OS applies when this process's handles close.</summary>
    TimedOut,
...
/// <see cref="AccelJobObject"/>'s kill-on-close, so "timed out" still is not "orphaned".</para>
///
...
public sealed class PtyShutdownCoordinator : IDisposable
{
    ...
    /// <see cref="AccelJobObject"/>'s handle.)
    /// </summary>
