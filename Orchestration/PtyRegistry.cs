...
/// or was closed by Accel.</summary>
public sealed class PtySessionEndedEventArgs : EventArgs
{
    ...
    /// <see cref="PtySessionExitReason.TornDown"/> = Accel closed it, so the UI already knows.
    /// </summary>
...
/// anything</b>; <see cref="AccelJobObject.Shared"/>'s kill-on-close is the remaining backstop.</para>
///
...
public sealed class PtyRegistry : IPtySessionHost, IDisposable
{
    ...
    /// <see cref="AccelJobObject.Shared"/>: a <c>static readonly</c> <see cref="Lazy{T}"/> field is a GC
    /// root for the life of the AppDomain, which is exactly the "app-lifetime singleton" lifetime the plan
    /// asks for. Unlike <see cref="AccelJobObject.Shared"/> this one <i>is</i> meant to be disposed
    ...
    public void Dispose()
    {
        ...
        try
        {
            ...
            {
                // Nothing further to do here: every child still standing is in AccelJobObject.Shared, and
