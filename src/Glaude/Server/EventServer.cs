...
public class EventServer
{
    ...
    /// per request. Not persisted to disk; see <see cref="Glaude.Metrics.SessionState"/>.
    ...
    /// (`glaude run --dump-raw &lt;dir&gt;`): when non-null, every received event's raw
    ...
    {
        string body = await ReadBodyAsync(ctx.Request);
        SafePrint(() => EventPrinter.PrintStatusLine(body));
    ...
    {
        string body = await ReadBodyAsync(ctx.Request);
        SafePrint(() => EventPrinter.PrintSubagentStatusLine(body));
