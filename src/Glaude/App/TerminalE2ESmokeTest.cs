...
/// <item>(c) raw Ctrl+C byte: <c>window.glaudeSimulateInput</c> (the exact function xterm.js's real
...
/// <item>integer cell metrics: <c>window.glaudeCellMetrics()</c> read back to confirm the pinned
...
{
    private const string Marker = "GLAUDE_TERMINAL_E2E_OK";
    ...
    private static void RunOnStaThread(TextWriter output, Action<bool> reportResult)
    {
        ...
        var window = new Window
        {
            ...
            Height = 500,
            ShowInTaskbar = false,
            Title = "glaude-terminal-e2e-smoke-test",
        };
    ...
    {
        string json = await terminal.Browser.CoreWebView2.ExecuteScriptAsync("JSON.stringify(window.glaudeTermSize())");
    ...
    /// window.glaudeSimulateInput, which calls that exact function - see terminal.js), arrives at the
        ...
        TextWriter output, TerminalView terminal, EventServer server, int port)
    {
        ...
        string script = $"window.glaudeSimulateInput({JsonSerializer.Serialize(CtrlC)});";
    ...
    private static async Task<bool> CheckIntegerCellMetricsAsync(TextWriter output, TerminalView terminal)
    {
        ...
        string outer = await terminal.Browser.CoreWebView2.ExecuteScriptAsync("JSON.stringify(window.glaudeCellMetrics())");
        string? inner = JsonSerializer.Deserialize<string>(outer);
        output.WriteLine($"  window.glaudeCellMetrics() raw: {inner}");

        if (string.IsNullOrEmpty(inner) || inner == "null")
        {
            output.WriteLine("  [FAIL] glaudeCellMetrics() returned null - xterm's internal render surface was unavailable");
            return false;
        }
    ...
    {
        string json = await terminal.Browser.CoreWebView2.ExecuteScriptAsync("window.glaudeSocketState()");
        return JsonSerializer.Deserialize<int>(json);
    }

    private static async Task<string> ReadReceivedTextAsync(TerminalView terminal)
    {
        string json = await terminal.Browser.CoreWebView2.ExecuteScriptAsync("window.glaudeReceivedText");
