// P2-T5b: Glaude's own glue script (NOT vendored - see THIRD_PARTY_NOTICES.txt for what is).
...
(function () {
  ...
  function createTerminal() {
    term = new Terminal({
      ...
      // off a live instance via window.glaudeCellMetrics(), before and after the correction below.
      fontSize: 14,
      lineHeight: 1,
      letterSpacing: 0,
    ...
    // window.glaudeCellMetrics() for how this is verified to actually work, with real numbers,
    // rather than assumed.
    ...
    // casing. window.glaudeSimulateInput (below) calls this exact function too, so the Ctrl+C
  ...
  window.glaudeAttachPty = function (tabId, port) {
    if (!term) {
      createTerminal();
    }
    ...
    // WebSocket upgrade to an arbitrary local TCP listener, and "glaude-terminal" is not a real,
    // DNS- or hosts-file-resolvable host, so "wss://glaude-terminal/..." would fail outright. The
    ...
    // "Origin: https://glaude-terminal" on the upgrade request regardless of the connection
    // target, because Origin reflects the *page's* origin (this script runs on a page navigated
    // to https://glaude-terminal/index.html) - exactly what PtyRoutes.ExpectedOrigin checks.
    ...
    mySocket.onmessage = function (event) {
      ...
      if (typeof event.data === "string") {
        window.glaudeReceivedText += event.data;
  ...
  window.glaudeSimulateInput = function (text) {
  ...
  window.glaudeCellMetrics = function () {
    try {
      var dimensions = term._core._renderService.dimensions;
      return {
  ...
  window.glaudeSocketState = function () {
  ...
  // runs in the page's own global scope, not inside this IIFE) - window.glaudeResizeTerminal is
  // the one exposed entry point a test needs to drive a resize the same way FitAddon would.
  window.glaudeSendResize = sendResize;
  window.glaudeResizeTerminal = function (cols, rows) {
    if (term) {
  ...
  // reason glaudeResizeTerminal exists) - used by terminal-e2e-smoke-test to confirm a resize
  // was actually applied client-side before asserting it reached the child.
  window.glaudeTermSize = function () {
    return term ? { cols: term.cols, rows: term.rows } : null;
  };

  try {
    createTerminal();
    document.title = "glaude-terminal-ready";
  } catch (e) {
    document.title = "glaude-terminal-error:" + e.message;
  }
})();
