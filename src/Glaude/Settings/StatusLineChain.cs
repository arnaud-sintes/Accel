...
/// <summary>
/// The two independent top-level status-line fields Glaude may take over.
...
/// <summary>
/// What was in a status-line field <b>before</b> Glaude took it over.
///
...
public static class StatusLineChain
{
    ...
    public static bool IsGlaudeOwned(StatusLineField field, JsonNode? node)
    {
        var command = GlaudeHookSpec.GetCommand(node);
        return field == StatusLineField.SubagentStatusLine
            ? GlaudeHookSpec.IsGlaudeSubagentStatusLineCommand(command)
            : GlaudeHookSpec.IsGlaudeStatusLineCommand(command);
    }
    ...
    /// current value is already Glaude's own — never overwrite a real capture with our own value.
    /// </summary>
    public static void Install(JsonObject root, StatusLineField field, JsonObject desired, IStatusLineChainStore store)
    {
        ...
        var currentIsOurs = IsGlaudeOwned(field, current);
    ...
    /// no longer Glaude-owned (someone else took it over after us) is left untouched.
    /// </summary>
