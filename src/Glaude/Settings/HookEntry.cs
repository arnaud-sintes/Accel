...
///
/// Glaude always uses <b>exec form</b> (<c>command</c> = the executable, <c>args</c> = an
...
{
    /// <summary>Header arg prefix that marks an entry as Glaude-owned (project.md "Marker scheme").</summary>
    public const string MarkerHeaderPrefix = "X-Glaude-Hook:";
    ...
    /// <summary>
    /// Returns the event name carried by this entry's Glaude marker header arg
    /// (<c>-H "X-Glaude-Hook: SessionStart"</c>), or null if the entry is not Glaude-owned.
    /// This is the only ownership test — never assume Glaude owns a whole event or the
    /// whole <c>hooks</c> object.
    /// </summary>
    public static string? GetGlaudeMarkerEvent(JsonNode? entryNode)
    {
    ...
    /// <summary>True if this entry carries the Glaude marker header arg.</summary>
    public static bool IsGlaudeOwned(JsonNode? entryNode) => GetGlaudeMarkerEvent(entryNode) is not null;
...
/// Glaude always installs its own <i>additional</i> group; existing groups belonging to other
