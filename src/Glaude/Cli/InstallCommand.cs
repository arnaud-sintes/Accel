...
/// <summary>
/// `glaude install` — loads the real settings.json, builds the expected
/// <see cref="GlaudeHookSpec"/> for the current port + exe path, version-gates
/// SubagentStart/subagentStatusLine, and applies via <see cref="SettingsMerger.InstallInto"/>.
///
/// Never crashes or corrupts the file on any error: all safety (atomic write, `.glaude.bak`
...
public static class InstallCommand
{
    ...
    public static int Run(int port, TextWriter output) =>
        Run(
            port,
            GlaudePaths.DefaultSettingsPath(),
            GlaudePaths.CurrentExePath(),
        ...
        Func<ClaudeVersion?> versionProbe)
    {
        ...
        var version = GlaudePaths.SafeProbe(versionProbe);
        ...
        var spec = new GlaudeHookSpec(port, exePath, includeSubagentStart, includeSubagentStatusLine);
    ...
    private static void PrintAppliedSummary(
        ...
        SettingsFile file)
    {
        output.WriteLine($"Installed Glaude into '{settingsPath}' (port {port}).");
        output.WriteLine(
        ...
        {
            output.WriteLine($"  Stray Glaude hooks removed (no longer expected): {string.Join(", ", before.StrayEvents)}");
        }
    ...
    private static void PrintStatusLineChange(TextWriter output, string fieldName, StatusLineOwnership before)
    {
        switch (before)
        {
            ...
            case StatusLineOwnership.None:
                ...
                break;
            case StatusLineOwnership.Glaude:
                output.WriteLine($"  {fieldName}: rewritten (already Glaude-owned).");
                break;
        }
    }
}
