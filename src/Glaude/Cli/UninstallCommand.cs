...
/// <summary>
/// `glaude uninstall` — loads the real settings.json, removes every Glaude-tagged hook entry
...
public static class UninstallCommand
{
    ...
    public static int Run(TextWriter output) =>
        Run(GlaudePaths.DefaultSettingsPath(), FileBackedStatusLineChainStore.DefaultPath(), output);
    ...
    public static int Run(string settingsPath, string statePath, TextWriter output)
    {
        ...
        if (!changed)
        {
            output.WriteLine("Nothing to uninstall: no Glaude entries were found.");
            return 0;
        }

        file.Save(file.Root!);

        output.WriteLine($"Uninstalled Glaude from '{settingsPath}'.");
