...
/// <summary>How <see cref="PtyOrphanReconciler"/> classified one <c>glaude-sessions.json</c> entry.</summary>
public enum PtyOrphanKind
{
    ...
    /// the same <c>claude</c> process a previous Glaude run launched, still running now. An
    ...
    /// for killing - i.e. Glaude offering to kill an unrelated process.
    /// </summary>
    Stale,
}

/// <summary>One classified registry entry, with a human-readable reason (for logging/UI, never parsed).</summary>
/// <param name="Entry">The <c>glaude-sessions.json</c> entry.</param>
...
public enum PtyOrphanActionOutcome
{
    ...
    /// <summary>"Adopt as detached": the process is left running, and Glaude stopped tracking it.</summary>
    Detached,
}
...
/// <summary>
/// P3-T4, second half: <b>startup orphan reconciliation</b>. Reads <c>glaude-sessions.json</c> (via
...
/// process a previous Glaude run launched that outlived that run. Every session is assigned to
/// <see cref="GlaudeJobObject"/> with <c>JOB_OBJECT_LIMIT_KILL_ON_JOB_CLOSE</c>, and that fires when the
/// <i>job handle</i> closes - which includes a hard crash or a Task-Manager kill of Glaude, since the kernel
...
/// <list type="bullet">
/// <item>the job assignment or its rooting failed for that session (see <see cref="GlaudeJobObject"/>'s
...
/// <item>the registry file was written but the process outlived Glaude for any reason the job could not cover -
...
/// be using (possibly attached to from another Glaude instance - nothing here can tell the difference), and a
...
/// unrecoverable one Glaude deleted the record of, or a killed session.</para>
///
...
/// <c>glaude-sessions.json</c>, but it is Claude Code's own per-PID status file (the rename gate in P4-T2), it has
...
/// so reconciliation uses <c>glaude-sessions.json</c> alone. Cross-reading the status file would only add "and
...
public static class PtyOrphanReconciler
{
    ...
    /// </summary>
    /// <param name="entries">Entries as read from <c>glaude-sessions.json</c>.</param>
    ...
    ///
    /// <para>"Detached" is the honest description of what Glaude can offer today: the process is not in this
    /// run's <see cref="GlaudeJobObject"/> and there is no ConPTY handle to reattach to (a pseudoconsole cannot be
