...
public sealed class PtyLaunchSpec
{
    ...
    /// <summary>Working directory for the child. Null means "inherit Glaude's".</summary>
    public string? WorkingDirectory { get; init; }

    /// <summary>Environment variables to add/override (null value removes one). Null or empty means the
    /// child inherits Glaude's environment unchanged.</summary>
        ...
        foreach (var shimExtension in ShimExtensions)
        {
            if (string.Equals(extension, shimExtension, StringComparison.OrdinalIgnoreCase))
            {
                throw new PtySessionLaunchException(
                    ...
                    "point instead; that path is not implemented yet. Re-check `Get-Command claude -All` / `glaude doctor` " +
                    "on this machine.");
            }
        }
...
public sealed class PtySessionOptions
{
    ...
    /// <see cref="GlaudeJobObject.Shared"/> - the process-wide, statically rooted instance. Overriding it
    /// is for tests/diagnostics that want a job they can close on purpose.
    /// </summary>
    public GlaudeJobObject? JobObject { get; init; }
}
...
public enum PtySessionExitReason
{
    ...
    /// teardown having been requested by Glaude.</summary>
    ChildExited,

    /// <summary>The child ended after <see cref="PtySession.Dispose"/> had been called, i.e. Glaude tore
...
/// those grandchildren would survive Glaude being killed. <see cref="ConPtyLaunchSpec.CreateSuspended"/>
...
/// <see cref="GlaudeJobObject.Shared"/>, held in a <c>static readonly</c> field and therefore a GC root
...
/// do (the alternative, an unbounded buffer, means a runaway child grows Glaude's heap without limit;
...
public sealed class PtySession : IDisposable
{
    ...
    private readonly GlaudeJobObject _jobObject; // (b) above: keeps the job rooted while this lives.
    ...
    public GlaudeJobObject JobObject => _jobObject;
        ...
        ClaudeCliResolution? resolution = null)
    {
        ...
        switch (resolved.Kind)
        {
            case ClaudeCliResolutionKind.Missing:
                throw new PtySessionLaunchException(
                    "Cannot launch `claude`: it was not found on PATH. Install Claude Code, or run `glaude doctor` " +
    ...
    public static PtySession Start(PtyLaunchSpec spec, PtySessionOptions? options = null)
    {
        ...
        // Resolve the job object BEFORE spawning: GlaudeJobObject.Shared creates the job on first use,
        // and a failure there must not leave a suspended orphan behind.
        var jobObject = options.JobObject ?? GlaudeJobObject.Shared;
