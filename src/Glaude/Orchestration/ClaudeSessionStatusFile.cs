...
/// itself maintains per process (distinct from <see cref="PtyPidRegistry"/>'s <c>glaude-sessions.json</c>,
/// which is Glaude's own launch registry). P4-T1/T2 read this to gate slash-command injection on the
...
/// convention (<see cref="PtyPidRegistry"/>, <see cref="Glaude.Server.RootFoldersConfig"/>,
/// <see cref="Glaude.Cli.FileBackedStatusLineChainStore"/>) - missing/empty/malformed degrades to
