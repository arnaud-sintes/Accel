...
{
    /// <summary>
    /// The real, live <c>%USERPROFILE%\.claude\settings.json</c>. Per project.md, Accel
    ...
    /// <summary>
    /// Resolves the path to Accel's own running executable, so hooks call the real binary
    /// rather than e.g. "dotnet". Falls back through <see cref="Environment.ProcessPath"/> then
    /// <see cref="Process.MainModule"/>, and finally a bare "accel.exe" if both are unavailable
    /// (never throws).
    /// </summary>
