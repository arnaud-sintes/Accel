...
{
    /// <summary>Port of the local Accel server that receives the fire-and-forget POST.</summary>
    public int Port { get; init; } = AccelHookSpec.DefaultPort;
...
/// <summary>
/// <c>accel statusline</c> — the command Claude Code runs as its top-level <c>statusLine</c>.
///
...
public static class StatusLineCommand
{
                ...
                async () =>
                {
                    try
                    {
                        ...
                        using var request = new HttpRequestMessage(
                            HttpMethod.Post,
                            AccelHookSpec.BuildUrl(port, StatusLineRoute))
                        {
                            Content = content,
                        };
    ...
    private static string? TryGetChainedCommand(IStatusLineChainStore? store)
    {
        ...
        try
        {
            ...
            return AccelHookSpec.GetCommand(capture.Original);
        }
        catch
        {
            return null;
