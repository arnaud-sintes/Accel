...
/// <see cref="Accel.Cli.DebounceCoalescer"/>.
///
...
public class TelemetryFeedTests
{
    ...
    {
        var (feed, source, _, timer, published, _) = Build();
        source.ProjectsDirectory = Path.Combine(Path.GetTempPath(), $"accel-not-there-{Guid.NewGuid():N}");
    ...
    {
        var (feed, source, _, _, _, _) = Build();
        string dir = Path.Combine(Path.GetTempPath(), $"accel-feed-watch-{Guid.NewGuid():N}");
