...
public class RawPayloadCaptureTests : IAsyncLifetime
{
    private readonly string _dumpDir = Path.Combine(
        Path.GetTempPath(), "accel-dumpraw-tests-" + Guid.NewGuid().ToString("N"));
    ...
    {
        string noCaptureDir = Path.Combine(Path.GetTempPath(), "accel-dumpraw-should-not-exist-" + Guid.NewGuid().ToString("N"));
