...
public class CreateSessionDialogViewModelTests
{
    ...
    [Fact]
    public void BuildArguments_SelectedPermissionMode_AppearsBeforeExtraArgsTail()
    {
        var viewModel = new CreateSessionDialogViewModel
        {
            SelectedPermissionMode = Accel.App.Services.PermissionModeOption.BypassPermissions,
        ...
        Assert.Equal("bypassPermissions", arguments[Array.IndexOf(arguments, "--permission-mode") + 1]);
        Assert.True(
            Array.IndexOf(arguments, "--permission-mode") < Array.IndexOf(arguments, "--add-dir"),
            "The combo-driven flag must precede the free-text extra-args tail.");
    }
    ...
    public void SelectedPermissionModeChoice_ReflectsAndUpdatesSelectedPermissionMode()
    {
        ...
        Assert.Equal(Accel.App.Services.PermissionModeOption.None, viewModel.SelectedPermissionModeChoice.Value);

        viewModel.SelectedPermissionModeChoice = viewModel.PermissionModeChoices[2]; // BypassPermissions

        Assert.Equal(Accel.App.Services.PermissionModeOption.BypassPermissions, viewModel.SelectedPermissionMode);
    }
    ...
    public void Confirm_WorkingDirectoryDoesNotExist_SetsErrorMessageAndNeverStartsASession()
    {
        ...
        Assert.False(starterCalled);
        Assert.Null(viewModel.LastStartedSession);
        Assert.Contains(@"C:\this\path\does\not\exist\accel-test", viewModel.ErrorMessage);
    }

    [Fact]
    public void Confirm_BlankWorkingDirectory_FallsBackToUserProfile_NeverToNull()
    {
        // A null working directory would have PtySession/ConPtySession inherit Accel.exe's own
    ...
    {
        var workingDirectory = Directory.CreateTempSubdirectory("accel-create-session-wd-test-").FullName;
        try
        {
            var viewModel = new CreateSessionDialogViewModel(
