...
/// panel A's root add/remove UI. Every test uses a fixture <c>accel-folders.json</c> path under
/// the OS temp directory; the real durable-home config (<c>%USERPROFILE%\.claude\accel-folders.json</c>)
/// is never touched.
///
...
public class RootFolderEditorTests : IDisposable
{
    ...
    {
        string path = Path.Combine(Path.GetTempPath(), $"accel-folders-editor-test-{Guid.NewGuid():N}.json");
        _tempPaths.Add(path);
        return path;
    }
    ...
    {
        string path = Path.Combine(Path.GetTempPath(), $"accel-root-editor-test-{Guid.NewGuid():N}");
        _tempDirs.Add(path);
        return path;
    }
