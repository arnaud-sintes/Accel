...
public class CliTests
{
    ...
    public void Parse_NoArgs_DefaultsToStart()
    {
        ...
        Assert.Equal(AccelHookSpec.DefaultPort, parsed.Port);
    ...
    public void Install_EmptyFile_InstallsCleanly()
    {
        ...
        var statePath = Path.Combine(dir.Path, "accel-state.json");
        ...
        Assert.Contains("Installed Accel", writer.ToString());

        var reloaded = SettingsFile.Load(settingsPath);
        var spec = new AccelHookSpec(Port, ExePath, includeSubagentStart: false, includeSubagentStatusLine: false);
    ...
    {
        var dir = Path.Combine(Path.GetTempPath(), $"accel-cli-test-{Guid.NewGuid():N}");
        var settingsPath = Path.Combine(dir, "settings.json");
        var statePath = Path.Combine(dir, "accel-state.json");
    ...
    {
        var path = Path.Combine(Path.GetTempPath(), $"accel-state-missing-{Guid.NewGuid():N}.json");
    ...
    {
        public string Path { get; } = System.IO.Path.Combine(System.IO.Path.GetTempPath(), $"accel-cli-test-{Guid.NewGuid():N}");
