...
/// <see cref="Accel.Settings.SettingsFile"/>'s atomic writer, not a second mechanism),
...
public class PtyPidRegistryTests : IDisposable
{
    ...
    {
        string path = Path.Combine(Path.GetTempPath(), $"accel-sessions-test-{Guid.NewGuid():N}.json");
        _tempFiles.Add(path);
        return path;
    }
