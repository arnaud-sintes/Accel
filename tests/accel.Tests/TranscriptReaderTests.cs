...
public class TranscriptReaderTests : IDisposable
{
    ...
    {
        string path = Path.Combine(Path.GetTempPath(), $"accel-transcript-{Guid.NewGuid():N}.jsonl");
        _tempFiles.Add(path);
        return path;
    }
    ...
    {
        string path = Path.Combine(Path.GetTempPath(), $"accel-does-not-exist-{Guid.NewGuid():N}.jsonl");
