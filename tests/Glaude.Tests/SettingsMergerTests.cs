...
public class SettingsMergerTests
{
    ...
    private static GlaudeHookSpec Spec(int port = Port) => new(port, ExePath);
    ...
    {
        var path = Path.Combine(Path.GetTempPath(), $"glaude-test-{Guid.NewGuid():N}", "settings.json");
    ...
    public void Load_EmptyJsonObject_InstallsCleanly()
    {
        ...
        Assert.Equal(StatusLineOwnership.Glaude, detected.StatusLine);
        Assert.Equal(StatusLineOwnership.Glaude, detected.SubagentStatusLine);
    }
    ...
    public void Install_RealWorldFile_LeavesThirdPartyEntriesAndTopLevelKeysUntouched()
    {
        ...
        // Glaude's own entries are present and exec-form.
        var sessionStart = SingleGlaudeEntry(root, "SessionStart");
        ...
        Assert.Contains("X-Glaude-Hook: SessionStart", args);
        ...
        var sessionEnd = SingleGlaudeEntry(root, "SessionEnd");
    ...
    public void Install_ForeignStatusLine_IsCapturedThenRestoredOnUninstall()
    {
        ...
        // Overwritten with Glaude's own command.
        Assert.True(GlaudeHookSpec.IsGlaudeStatusLineCommand(root["statusLine"]!["command"]!.GetValue<string>()));
    ...
    public void Install_Twice_DoesNotOverwriteTheCapturedOriginal()
    {
        ...
        Assert.True(store.TryGet(StatusLineField.StatusLine, out var captured));
        Assert.Contains("ccstatus", GlaudeHookSpec.GetCommand(captured.Original));
    }
    ...
    public void Install_Twice_ProducesNoDuplicateEntries()
    {
        ...
        var glaude = SettingsMerger.EnumerateGlaudeEntries(root).ToList();
        Assert.Equal(4, glaude.Count);
        Assert.Equal(4, glaude.Select(g => g.EventName).Distinct().Count());
    }

    // ---- port drift ------------------------------------------------------------------

    [Fact]
    public void Detect_PortDrift_AndReinstallRewritesOnlyGlaudeEntries()
    {
        var root = Parse(RealWorldFixture);
        var store = new InMemoryStatusLineChainStore();
        ...
        // Every Glaude entry now carries the new port, and there are no leftovers on the old one.
        Assert.All(
            SettingsMerger.EnumerateGlaudeEntries(root),
    ...
    [Fact]
    public void Detect_StrayGlaudeEventNotInSpec_IsPartiallyInstalled_AndInstallRemovesIt()
    {
        var root = Parse(RealWorldFixture);
        var store = new InMemoryStatusLineChainStore();
        ...
        var gated = new GlaudeHookSpec(Port, ExePath, includeSubagentStart: false);
    ...
    public void Uninstall_PrunesEmptyContainersButKeepsOtherEvents()
    {
        ...
        // Glaude's own event keys are gone...
    ...
    [Fact]
    public void Uninstall_RemovesOnlyGlaudeEntryFromASharedMatcherGroup()
    {
        var root = Parse(RealWorldFixture);
        SettingsMerger.Install(root, Spec(), new InMemoryStatusLineChainStore());

        // Another tool appends its own entry into Glaude's matcher group.
        var glaudeGroup = root["hooks"]!["SessionStart"]!.AsArray()[0]!.AsObject();
        glaudeGroup["hooks"]!.AsArray().Add(new JsonObject
        {
            ["type"] = "command",
    ...
    {
        var dir = Path.Combine(Path.GetTempPath(), $"glaude-test-{Guid.NewGuid():N}");
    ...
    private static JsonObject SingleGlaudeEntry(JsonObject root, string eventName)
    {
        var located = SettingsMerger.EnumerateGlaudeEntries(root)
    ...
    private sealed class TempSettings : IDisposable
    {
        ...
        {
            directory = System.IO.Path.Combine(System.IO.Path.GetTempPath(), $"glaude-test-{Guid.NewGuid():N}");
