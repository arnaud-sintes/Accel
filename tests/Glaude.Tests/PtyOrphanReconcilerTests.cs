...
public class PtyOrphanReconcilerTests : IDisposable
{
    ...
    {
        string path = Path.Combine(Path.GetTempPath(), $"glaude-orphan-test-{Guid.NewGuid():N}.json");
        _tempFiles.Add(path);
        return path;
    }

    private static PtyPidEntry Entry(string sessionId, int pid, DateTime? startTime = null) =>
        new(sessionId, pid, startTime ?? Start, @"C:\projects\Glaude", Start.AddMinutes(-1));
