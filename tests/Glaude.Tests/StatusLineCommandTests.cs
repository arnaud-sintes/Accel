...
/// <summary>
/// Phase 5. The invariant under test everywhere below: <c>glaude statusline</c> stdout <i>is</i>
...
{
    /// <summary>A port nothing is listening on — stands in for "Glaude server is not running".</summary>
    ...
    private const string SamplePayload = """
        {
          ...
          "workspace": { "current_dir": "C:\\projects\\Glaude" },
          "version": "2.1.224"
        }
        """;
    ...
    public async Task NoChainedCommand_SynthesizesDefaultLineFromStdin()
    {
        ...
        Assert.Contains(@"C:\projects\Glaude", output);
    }
    ...
    {
        var store = StoreWith("echo GLAUDE-CHAIN-MARKER");
        ...
        Assert.Equal("GLAUDE-CHAIN-MARKER" + Environment.NewLine, output);
    ...
    {
        // The original status-line process is long gone and cannot re-read stdin, so Glaude must
    ...
    {
        var store = StoreWith("glaude-no-such-executable-xyz --nope");
    ...
    public async Task AlwaysExitsZero_AcrossEveryScenario()
    {
        var scenarios = new (string Payload, IStatusLineChainStore? Store)[]
        {
            ...
            (SamplePayload, StoreWith("glaude-no-such-executable-xyz")),
        };
