...
{
    private readonly string _fixtureRoot = Path.Combine(Path.GetTempPath(), $"glaude-roots-tree-{Guid.NewGuid():N}");
    ...
    public async Task HistoricalSession_ClaudeSonnet5_RealisticHighUsage_DoesNotExceed100Percent()
    {
        ...
        {
            ModeLine(),
            UserLine("Investigate the two real Glaude bugs", @"C:\projects"),
