...
public class TranscriptHeadReaderTests : IDisposable
{
    ...
    {
        string path = Path.Combine(Path.GetTempPath(), $"glaude-transcript-head-{Guid.NewGuid():N}.jsonl");
        _tempFiles.Add(path);
        return path;
    }
    ...
    {
        string path = Path.Combine(Path.GetTempPath(), $"glaude-head-does-not-exist-{Guid.NewGuid():N}.jsonl");
